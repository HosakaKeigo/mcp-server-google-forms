...
export class BatchUpdateFormTool {
   ...
   * ツールの説明
   */
  readonly description = "Google Formsの複数の更新操作を一括で実行します。項目の追加・更新・削除・移動などを一度に行えます。";
  ...
  readonly parameters = {
    ...
    operations: z.array(
      z.object({
        ...
        item_type: z.enum(["text", "question", "pageBreak", "questionGroup"]).optional()
          .describe("作成する項目のタイプ（create_item時に必要）"),

        // 質問タイプ情報（質問作成時）
        question_type: z.enum(["TEXT", "PARAGRAPH_TEXT", "RADIO", "CHECKBOX", "DROP_DOWN"]).optional()
        ...
        include_other: z.boolean().optional().describe("「その他」オプションを含めるか（選択式質問の場合）"),
    ...
    try {
      ...
      for (const [opIndex, op] of args.operations.entries()) {
        try {
          ...
          switch (op.operation) {
            ...
            case "move_item": {
              ...
              if (op.new_index < 0 || op.new_index > form.items.length) {
                throw new Error(`操作 #${opIndex + 1}: 新しいインデックス ${op.new_index} が範囲外です`);
              }
          ...
          throw new Error(`操作 #${opIndex + 1}でエラーが発生しました: ${error instanceof Error ? error.message : String(error)}`);
        }
      }
